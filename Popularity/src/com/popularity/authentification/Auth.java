...
public class Auth  {
	...
	public static Credential authorize(List<String> scopes,
		...
		GoogleClientSecrets clientSecrets = GoogleClientSecrets.load(
				JSON_FACTORY, clientSecretReader);
		...
		if (clientSecrets.getDetails().getClientId().startsWith("Enter")
				|| clientSecrets.getDetails().getClientSecret()
						.startsWith("Enter ")) {
			System.out.println("Checks that the defaults have been replaced");
			System.out
		...
		System.out
				.println("This creates the credentials datastore at ~/.oauth-credentials/${credentialDatastore}");
		...
		LocalServerReceiver localReceiver = new LocalServerReceiver.Builder()
				.setPort(8080).build();
		System.out.println("endauthorize");
		// Authorize.
